...
type Column struct {
	...
	PK    string `json:"-"` //在go代码中根据 IsKey 推导出 是"pk"或空
	// Omitempty string `json:"omit"`// json中omitempty是关键词，不能用于读取数据进来
	Omitempty string `json:"omitempty"` // json中omitempty是关键词，不能用于读取数据进来
	// 索引名字
	...
	//不为空
	Notnull string `json:"notnull"` // "notnull": "notnull",
	// 数据库对应的字段名字
	ColumnName string `json:"column_name"`
	//列类型
	ColumnType string `json:"column_type"` // 如果没传入值则在go代码中（非模板中）根据PropType推导出来
	//长度
	...
	//备注
	PropComment string `json:"prop_comment"`
	// 在数据库中生成comment
	CommentShowInDB bool `json:"comment_in_db"`
}

type Table struct {
	TableName  string   `json:"table_name"`
	PoName     string   `json:"po_name"`
	ColumnList []Column `json:"column_list"`
	//备注
	PoComment string `json:"po_comment"`
	// 在数据库中生成comment
	CommentInDB bool `json:"comment_in_db"`
}
